...
function mapItem(item){return{item_id:String(item.spu_id||item.sku_id||item.SKU||""),item_name:item.product_name||"",item_variant_id:String(item.sku_id||item.SKU||""),price:normalizePrice(item.price||item.final_price),quantity:item.quantity||1,item_category_id:CATEGORY_ID,image_url:item.image_url||getImageUrl()}}
